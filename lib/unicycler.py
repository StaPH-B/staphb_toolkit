...
class Unicycler:
    ...
    def __init__(self,threads=None,runfiles=None, path=None, arguments = "", output_dir = ""):
        if runfiles:
        ...
        #add additional arguments if specified at the initilization
    ...
    def assemble(self):
        for read in self.runfiles.reads:
            ...
            if self.path == self.output_dir:
                ...
                out_dir = '/data/{id}'.format(id=id)
                in_dir = '/data'
            else:
                mounting = {self.path:'/datain',self.output_dir:'/dataout'}
                out_dir = '/dataout/{id}'.format(id=id)
                in_dir = '/datain'

            #build command for running trimmomatic
            if self.runfiles.reads[read].paired:
                command = "unicycler -t {threads} -1 {in_dir}/{fwd} -2 {in_dir}/{rev} -o {out_dir} {arguments}".format(threads=self.threads,in_dir=in_dir,out_dir=out_dir,fwd=fwd,rev=rev,arguments=self.arguments)
            else:
                command = "unicycler -t {threads} -s {in_dir}/{fastq} -o {out_dir} {arguments}".format(threads=self.threads,in_dir=in_dir,out_dir=out_dir,fwd=fwd,rev=rev,arguments=self.arguments)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--args",default="", type=str, help="change additional arguments for unicycler")
    ...
    assem_obj = Unicycler(threads=threads,path=path,arguments=arguments,output_dir=output_dir)
    assem_obj.assemble()
