...
### Group-A Streptoccus (GAS) Analysis

The analysis of GAS isolates has been completed. These results must always be used in conjunction with epidemiological data when determining if isolates are epidemiologically linked. This analysis should not be used as a replacement for a thorough epidemiological investigation.
...
Using variation within the genome between samples (SNPs), we can estimate how related isolates are. We do this by determining if isolates share a similar common ancestor. Here we are looking for isolates that cluster together and share a small amount of horizontal distance on the tree. Bootstrap values are shown on the tree. A bootstrap value greater than 95 suggests the placement of a branch on the tree is well supported.
...
The figures shown here were generated using sequence data processed with our data analysis pipeline, [Foushee](https://github.com/StaPH-B/staphb_toolkit/tree/dev). Sequences were assembled using a de-novo approach [shovill](https://github.com/tseemann/shovill). SNP calls and parsimony tree [kSNP3](https://academic.oup.com/bioinformatics/article/31/17/2877/183216). The Pairwise-SNP matrix was snp-dists. The phylogenetic tree was generated from a whole genome alignment (link to mafft). The tree was generated using a maximum-likelihood method with the GTR+G4 substitution model and bootstrapped with 1000 replicates (link to iqtree). Report generated using R and designed by Abigail Shockey. If you have questions about this report please contact X at X.
